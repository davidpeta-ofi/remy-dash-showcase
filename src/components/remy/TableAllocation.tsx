...
const ReservationCard: React.FC<{
  ...
  r
}) => <div draggable onDragStart={e => e.dataTransfer.setData("text/plain", r.id)} aria-label={`Reservation ${r.name} party of ${r.size} at ${r.time}`} className="rounded-md border border-border p-2 text-card-foreground text-sm cursor-grab active:cursor-grabbing bg-sky-400">
            ...
            <div className="md:col-span-2">
              ...
              height: 360
                ...
                <div className="absolute inset-3 grid grid-cols-5 grid-rows-6 gap-3 place-items-center">
                  {tables.map(t => <div key={t.id} onDragOver={e => e.preventDefault()} onDrop={e => {
                  e.preventDefault();
                  const rid = e.dataTransfer.getData("text/plain");
                  if (rid) {
                    toast({
                      title: `Placed reservation on Table ${t.id}`,
                      description: "Static demo only"
                    });
                  }
                }} aria-label={`Table ${t.id} with ${t.seats} seats`} className={`${t.shape === "circle" ? "rounded-full" : "rounded-xl"} h-full w-full flex items-center justify-center border shadow-sm ${isReserved(t.id, selectedDate) ? "bg-secondary/20 border-secondary text-secondary-foreground" : "bg-accent/15 border-accent text-accent-foreground"}`}>
